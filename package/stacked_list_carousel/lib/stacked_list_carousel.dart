...
class StackedListCarousel<T> extends StatefulWidget {
  const StackedListCarousel({
    ...
    Key? key,
        ...
        assert(
          outermostCardHeightFactor + (maxDisplayedItemCount - 1) * itemGapHeightFactor <= 1,
...
class _StackedListCarouselState<T> extends State<StackedListCarousel<T>> with TickerProviderStateMixin {
  ...
  void updateAnimations({
    ...
    cardsSizeFactors
      ...
      ..addAll(
        List.generate(
          maxDisplayedItemCount,
          (index) => widget.outermostCardHeightFactor - widget.itemGapHeightFactor * (maxDisplayedItemCount - index - 1),
        ),
      );
  ...
  Widget _innermostCard(
    ...
    final child = cards[(reorderCardsCount + controller.swapCount + 1) % controller.itemCount] ?? const SizedBox.shrink();

    return AnimatedBuilder(
      animation: innermostCardMarginAnimation,
      builder: (context, child) => _alignPositioned(
        margin: (controller.transitionForwarding ? innermostCardMarginAnimation.value : cardsMargin.first) * size.height,
  ...
  Widget _innerCard(
    ...
    final child = cards[(reorderCardsCount - i + controller.swapCount) % controller.itemCount] ?? const SizedBox.shrink();
          ...
          child: AnimatedBuilder(
            ...
            builder: (context, _) => Transform.scale(
              ...
              child: Visibility(
                ...
                child: hasOutermostWrapper ? widget.outermostCardWrapper!.call(child) : child,
              ),
            ),
          ),
        ),
  ...
  Widget _indexedCard(
    ...
    return SizedBox.fromSize(
      size: cardSize,
      child: widget.cardBuilder.call(context, controller.items[index], cardSize),
    );
  }
